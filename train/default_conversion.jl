using BTNNs: RLayer, convert2discrete
...
history = train_default!(model,AdaBelief(), train_data, test_data; epochs=30)
...
writedlm("/history/default_conversion.csv", history)
...
function show_all_accuracies(history, show_legend=true)
    f = Figure()
    ax1 = Axis(f[1, 1], xlabel="epoch", ylabel="validation accuracy", title="Accuracies of the continuous and the converted (discrete) model on the validation dataset")
